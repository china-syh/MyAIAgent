...
// 动态调色板：根据分镜内容生成不同的颜色
const getPanelColor = (sceneNumber: number, panelNumber: number, composition: string = '') => {
  const baseHues = [260, 240, 280, 220, 300, 200, 320, 180];
  const hueIndex = (sceneNumber * 7 + panelNumber * 3) % baseHues.length;
  let baseHue = baseHues[hueIndex];

  // 根据构图类型微调色相
  if (composition.includes('战斗') || composition.includes('广角')) baseHue = 0;       // 红色系
  else if (composition.includes('特写')) baseHue = 200;                                 // 蓝色系
  else if (composition.includes('全景')) baseHue = 120;                                 // 绿色系
  else if (composition.includes('对话') || composition.includes('双人')) baseHue = 40;  // 橙色系

  return `hsl(${baseHue}, 60%, ${50 + (panelNumber % 3) * 8}%)`;
};

const getPanelGradient = (sceneNumber: number, panelNumber: number, composition: string = '') => {
  const c1 = getPanelColor(sceneNumber, panelNumber, composition);
  const c2 = getPanelColor(sceneNumber, panelNumber + 1, composition);
  return `linear-gradient(135deg, ${c1}22, ${c2}22)`;
};
  ...
  const storyboardContent = storyboards.length > 0 ? (
    <div>
      {Object.entries(groupedByScene).map(([sceneNum, panels]) => {
        const sceneColor = getPanelColor(parseInt(sceneNum), 1);
        return (
          <div key={sceneNum} style={{ marginBottom: 24 }}>
            <div style={{
              background: `linear-gradient(135deg, ${sceneColor}15, ${sceneColor}05)`,
              padding: '8px 16px',
              borderRadius: 8,
              marginBottom: 12,
              borderLeft: `4px solid ${sceneColor}`,
            }}>
              ...
              {panels[0]?.composition && (
                <Tag color="purple" style={{ marginLeft: 8 }}>
                  {panels[0].composition} · {panels[0].camera_angle}
                </Tag>
              )}
            </div>
            <Row gutter={[16, 16]}>
              {panels.map((sb, idx) => {
                const panelColor = getPanelColor(parseInt(sceneNum), idx + 1, sb.composition);
                ...
                return (
                  <Col span={8} key={`${sceneNum}-${idx}`}>
                    <Card
                      ...
                      title={
                        <Space>
                          <CameraOutlined style={{ color: panelColor }} />
                          <span>第 {sb.panel_number} 格</span>
                          <Tag style={{ fontSize: 10, background: `${panelColor}22`, border: 'none', color: panelColor }}>
                            {sb.composition}
                          </Tag>
                        </Space>
                      }
                      extra={<Tag color="purple" style={{ fontSize: 10 }}>{sb.camera_angle}</Tag>}
                    >
                      {/* 图片区域 */}
                      <div
                        style={{
                          ...
                          overflow: 'hidden',
                          position: 'relative',
                          background: !hasImage ? getPanelGradient(parseInt(sceneNum), idx + 1, sb.composition) : undefined,
                      ...
                      >
                        {hasImage ? (
                          ...
                          <Space direction="vertical" align="center">
                            <LoadingOutlined style={{ fontSize: 28, color: panelColor }} />
                      ...
                      <Descriptions column={1} size="small" style={{ marginBottom: 8 }}>
                        <Descriptions.Item label="构图">
                          <Tag color="blue">{sb.composition}</Tag>
                        </Descriptions.Item>
                        <Descriptions.Item label="角度">
                          <Tag color="cyan">{sb.camera_angle}</Tag>
                        </Descriptions.Item>
                      </Descriptions>
          ...
          >
            ...
            <div style={{ marginTop: 8 }}>
              <Descriptions column={2} size="small">
                ...
                <Descriptions.Item label="角度">{sb.camera_angle}</Descriptions.Item>
                <Descriptions.Item label="对话">{sb.dialogue || '-'}</Descriptions.Item>
              </Descriptions>
            </div>
          </Card>
        ))
      ...
      >
        {currentScript ? (
          <Descriptions column={2} bordered size="small">
            ...
            <Descriptions.Item label="构图类型">
              <Text>{[...new Set(storyboards.map(s => s.composition))].filter(Boolean).join('、') || '-'}</Text>
            </Descriptions.Item>
          </Descriptions>
        ) : (
